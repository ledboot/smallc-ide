          ...
          {network && (
            ...
            >
              <SelectTrigger className="h-6 border-none bg-transparent hover:bg-transparent p-0 gap-1 focus:ring-0 shadow-none">
                ...
                >
                  {network.icon && (
                    <img src={network.icon} alt="" className="w-3 h-3 rounded-full" onError={(e) => (e.currentTarget.style.display = 'none')} />
                  )}
              ...
              <SelectContent align="end" className="p-1 min-w-35">
                {networks.map((net: ZentNetwork) => (
                  <SelectItem key={net.chainId} value={net.chainId.toString()} className="text-xs font-semibold py-2">
                    <div className="flex items-center gap-2">
                      {net.icon && (
                        <img src={net.icon} alt="" className="w-4 h-4 rounded-full" onError={(e) => (e.currentTarget.style.display = 'none')} />
                      )}
                      {net.name}
                    </div>
                  </SelectItem>
